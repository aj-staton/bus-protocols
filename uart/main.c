...
int main(void) {
	...
	while (1) {
                /*
		Do we use get/put_char()? If yes, what's the param?
		Does UDRO give you a characer in the buffer?
 		Do our strcmps need a \n?
  		Why is the terminal output strange?
		*/
		_delay_ms(INTERVAL_MS);
		printf("cursor=%3u buf='%s'\n", cursor, buf);

		// Read in register to buffer  
		if (UCSR0A & (1 << RXC0) == 0){ 
			// until \n is found, fill buffer
			buf[cursor] = UDR0;
			cursor++;
			

			//  Evaluate Buffer - Do we want our comparison nested or not?? (more cycles)  
	    		if (strcmp(buf,"on\n") ==  0) {
		...
		// EXECUTE FUNCTION BASED ON SET STATE 
		if (state == 0) { //dont blink
			// set the thing to 0  
			PORTC &= 0;
